...
fun MdvConnectionTelemetryCard(
    ...
    MdvCardLow(modifier = Modifier.fillMaxWidth()) {
        Column(modifier = Modifier.fillMaxWidth().padding(MdvSpace.S3)) {
            ...
            Text(
                text = stringResource(R.string.home_traffic_totals, formatBytes(downTotalBytes), formatBytes(upTotalBytes)),
                style = MaterialTheme.typography.bodySmall,
                color = MdvColor.OnSurfaceVariant
            )
            if (connectionDurationText.isNotBlank()) {
                Text(
                    text = stringResource(R.string.home_connection_duration, connectionDurationText),
                    style = MaterialTheme.typography.bodySmall,
                    color = MdvColor.OnSurfaceVariant
                )
            }
...
@Composable
fun MdvPreConnectChecklist(warnings: List<String>) {
    if (warnings.isEmpty()) return
    MdvCardLow(modifier = Modifier.fillMaxWidth()) {
        Column(modifier = Modifier.fillMaxWidth().padding(MdvSpace.S3)) {
            Text(
                text = stringResource(R.string.home_preconnect_title),
...
fun MdvErrorCard(msg: String) {
    MdvCardLow(modifier = Modifier.fillMaxWidth()) {
        Column(modifier = Modifier.padding(MdvSpace.S3)) {
            ...
            errorSuggestion(msg)?.let { suggestion ->
                androidx.compose.foundation.layout.Spacer(modifier = Modifier.height(MdvSpace.S1))
                Text(
                    text = suggestion,
                    style = MaterialTheme.typography.bodySmall,
...
@Composable
private fun errorSuggestion(message: String): String? {
    return when {
        message.contains("SOCKS5", ignoreCase = true) -> stringResource(R.string.home_error_suggestion_socks)
        message.contains("port", ignoreCase = true) -> stringResource(R.string.home_error_suggestion_port)
        message.contains("resolver", ignoreCase = true) -> stringResource(R.string.home_error_suggestion_resolver)
        message.contains("permission", ignoreCase = true) -> stringResource(R.string.home_error_suggestion_permission)
        else -> null
    }
}
