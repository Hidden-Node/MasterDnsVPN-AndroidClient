...
object ConfigGenerator {
    ...
    /**
     * Generate a shareable TOML string from a ProfileEntity.
     * If [lockIdentity] is true, domains and encryptionKey are encrypted.
     */
    fun exportToml(profile: ProfileEntity, lockIdentity: Boolean = false): String {
        val domains = parseDomains(profile.domains)
        val domainsToml: String
        val keyToml: String

        if (lockIdentity) {
            domainsToml = "\"${IdentityCipher.encrypt(domains.joinToString(","))}\""
            keyToml = IdentityCipher.encrypt(profile.encryptionKey)
        } else {
            domainsToml = domains.joinToString(", ") { "\"${escapeToml(it)}\"" }
            keyToml = escapeToml(profile.encryptionKey)
        }

        val advanced = parseAdvanced(profile.advancedJson)
        fun cfg(key: String): String = advanced[key]?.trim().orEmpty()

        return buildString {
            appendLine("# MasterDnsVPN Profile Export")
            appendLine("# Exported by MasterDnsVPN Android")
            if (lockIdentity) {
                appendLine("# IDENTITY_LOCKED = true (domains and key are encrypted)")
            }
            appendLine()

            if (lockIdentity) {
                appendLine("DOMAINS = $domainsToml")
            } else {
                appendLine("DOMAINS = [$domainsToml]")
            }
            appendLine("DATA_ENCRYPTION_METHOD = ${profile.encryptionMethod}")
            appendLine("ENCRYPTION_KEY = \"$keyToml\"")
            if (lockIdentity) appendLine("IDENTITY_LOCKED = true")
            appendLine()

            appendLine("PROTOCOL_TYPE = \"${escapeToml(profile.protocolType)}\"")
            appendLine("LISTEN_PORT = ${profile.listenPort}")
            ...
            appendLine("LOG_LEVEL = \"${escapeToml(profile.logLevel)}\"")
            appendLine()

            EXPORT_ADVANCED_KEYS.forEach { key ->
                val value = cfg(key)
                if (value.isNotEmpty()) {
    ...
    fun generateConfig(
        ...
        val domains = parseDomains(profile.domains).map { domain ->
            if (IdentityCipher.isEncrypted(domain)) IdentityCipher.decrypt(domain) ?: domain else domain
        }
        val domainsStr = domains.joinToString(", ") { "\"$it\"" }
        val encryptionKey = if (IdentityCipher.isEncrypted(profile.encryptionKey)) {
            IdentityCipher.decrypt(profile.encryptionKey) ?: profile.encryptionKey
        } else {
        ...
        return buildString {
            ...
            appendLine("DATA_ENCRYPTION_METHOD = ${profile.encryptionMethod}")
            appendLine("ENCRYPTION_KEY = \"${escapeToml(encryptionKey)}\"")
            appendLine()
    ...
    private val EXPORT_ADVANCED_KEYS = listOf(
